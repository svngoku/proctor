...
def call_llm(
    ...
    if not config.get("api_key"):
        ...
        raise LLMError("Missing API key. Please set OPENROUTER_API_KEY environment variable.")
    ...
    while attempts <= max_retries:
        try:
            ...
            if response.choices and response.choices[0].message:
                ...
                litellm.exceptions.ServiceUnavailableError) as e:
            ...
            if attempts <= max_retries:
                ...
                log.warning(f"Retryable error: {str(e)}. Retrying in {retry_delay}s... (Attempt {attempts}/{max_retries})")
                import time
