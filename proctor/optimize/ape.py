...
class APEOptimizer(PromptOptimizer):
    ...
    async def optimize(
        ...
        **kwargs,
        ...
        """
        log.info(
            f"[bold cyan]Starting APE optimization for task: {task_description}[/]"
        )
        ...
        scored_candidates = await self._evaluate_candidates(
            candidates, labeled_examples, technique, eval_batch_size
        )
    ...
    async def _generate_candidates(
        ...
        demos = "\n".join(
            [f"Input: {ex['input']}\nOutput: {ex['output']}" for ex in demo_examples]
        )
...
Output each instruction on a new line, numbered 1 to {num_candidates}:"""
        ...
        try:
            ...
            while len(candidates) < num_candidates:
                candidates.append(
                    random.choice(seed_prompts)
                    if seed_prompts
                    else f"Please {task_description}"
                )
        ...
        for i in range(0, len(candidates), batch_size):
            ...
            for candidate in batch:
                task = self._evaluate_single_candidate(
                    candidate, labeled_examples, technique
                )
            ...
            log.info(
                f"Evaluated batch {i // batch_size + 1}/{(len(candidates) + batch_size - 1) // batch_size}"
            )
